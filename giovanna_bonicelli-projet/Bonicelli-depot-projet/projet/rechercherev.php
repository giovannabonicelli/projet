...
if (isset($_POST['nom'])AND isset($_POST['prenom']) )
{
	try
	{
		$etud=$bdd->query("SELECT nom, prenom, nom_ev FROM etudiant, coopere, evenement WHERE evenement.numero_ev=coopere.numero_ev AND etudiant.ine=coopere.ine AND nom='".$_POST['nom']."' AND prenom='".$_POST['prenom']."';");
		$count=$etud ->rowCount();
		if ($count>='1'){
			$tab = $etud->fetchAll();
			echo htmlspecialchars($_POST['nom'].' '.$_POST['prenom'].' :').'<br />';
			foreach($tab as $value){
				echo ($value['nom_ev'].'<br />') ;
			}
		}
		else {
